    ...
    namespace MDP {
        ...
        class SparseModel {
            public:
                ...
                SparseModel(const M& model, bool precision=false);
        ...
        SparseModel::SparseModel(const M& model, bool precision /* =false */) : S(model.getS()), A(model.getA()), discount_(model.getDiscount()), transitions_(A, SparseMatrix2D(S, S)), rewards_(A, SparseMatrix2D(S, S)),
                                       rand_(Impl::Seeder::getSeed())
        {
	  // Default AIToolbox constructor
	  if ( !precision ) {
	  ...
	  // otherwise use kahan sum for more precision
	    ...
	    std:: cerr << "    > Building SparseModel with Kahan summation\n";
		  ...
		  if ( checkDifferentSmall(0.0, p) ) {
		    transitions_[a].insert(s, s1) = p;
		    double val = p - kahan_correction;
		    double aux = kahan_sum + val;
		    kahan_correction = (aux - kahan_sum) - val;
		    kahan_sum = aux;
		  }
		  double r = model.getExpectedReward(s, a, s1);
		  if ( checkDifferentSmall(0.0, r) ) rewards_[a].insert(s, s1) = r;
                }
		// Replacing transitions_[a].row(s).sum() by kahan_sum for precision
                if ( checkDifferentSmall(1.0, kahan_sum) ) throw std::invalid_argument("Input transition table does not contain valid probabilities.");
	      }
	  }
        }
